...
module.exports = class HelpCommand extends Command {
    ...
    async run(message, args) {
        ...
        `)
                .setFooter({
                    text: `Expires in 60 seconds \nIf you'd like to report a bug please, type ${prefix}report \n` + message.member.displayName,
            ...
            collector.on('collect', b => {
                ...
                tempEmbed.fields = []
                tempEmbed = tempEmbed.setFooter({
                    text: `Expires in 60 seconds \nIf you'd like to report a bug please, type ${prefix}report \n` + message.member.displayName,
                    iconURL: message.author.displayAvatarURL()
                }).setTimestamp().setColor("RANDOM")

                if (type === `Slash Only Commands`) tempEmbed.addField(`${emojis.verified_developer} **/${type}**`, `` + commands[type])
                else tempEmbed.addField(`**${emojiMap[type]} [${commands[type].length}]**`, `${emojiMap[type].includes('Admin') ? 'Commands can be cleared by replacing "set" with "clear".\ni.e `setmodlog` ➔ `clearmodlog`\n-----------------------------------------------------\n' : ''} ${commands[type].join(', ')}`);
