...
module.exports = class SoftBanCommand extends Command {
    ...
    async run(message, args) {
        ...
        message.channel.send({
            embeds: [new MessageEmbed().setTitle('Softban Member')
                .setDescription(`Do you want to softban ${member}?`).setFooter({text: `Expires in 15s`})],
            components: [row]
        }).then(async msg => {
            ...
            collector.on('collect', async b => {
                ...
                if (b.customId === 'proceed') {
                    ...
                    this.done(message.author.id)
                    msg.edit({
                        components: [], embeds: [new MessageEmbed().setTitle('Soft Ban Member')
                            .setDescription(`${member} Not softbanned - Cancelled`)]
                    });
                }
            })
