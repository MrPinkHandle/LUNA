...
module.exports = class RoleCommand extends Command {
    constructor(client) {
        super(client, {
            ...
            examples: ['role @PinkyPeaker rolename', 'role @PinkyPeaker rolename, +rolename2, -rolename3']
        });
    }
    ...
    async run(message, args) {
        ...
        args = args.map(arg => {
            let role = arg.trim()
            if (role.startsWith(',') || role.endsWith(',')) role = role.replace(',', '');
            return role
        })
    ...
    async RemoveRole(member, role, message) {
        if (role.editable) {
            try {
                ...
                return this.sendErrorMessage(message, 1, `Please check the role hierarchy\n${err.message}`, err.message);
            }
        }
    }

    async addRole(member, role, message, failed) {
        if (role.editable) {
            try {
