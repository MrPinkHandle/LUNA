...
module.exports = class betCommand extends Command {
    constructor(client) {
        super(client, {
            ...
            examples: ['bet @PinkyPeaker#9165 1000'],
            exclusive: true
        });
    }
    ...
    run(message, args) {
        ...
        if (amount < 0 || amount > points) {
            this.done(message.author.id)
            return message.reply(`${emojis.nep} Please provide an amount you currently have! You have ${points} points ${emojis.point}`).then(m => setTimeout(() => m.delete(), 5000));
        }
        ...
        row.addComponents(new MessageButton().setCustomId(`proceed`).setLabel(`✅ Accept`).setStyle('SUCCESS'))
        row.addComponents(new MessageButton().setCustomId(`cancel`).setLabel(`❌ Decline`).setStyle('DANGER'))
        ...
        try {
            message.channel.send({
                ...
                collector.on('end', () => {
                    ...
                    this.done(member.user.id)
                    if (updated) return;
                    msg.edit({components: [], content: `${member} did not accept the bet - Expired`});
                });
            })
        } catch (e) {
            console.log(e)
